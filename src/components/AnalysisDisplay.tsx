...
import { DownloadIcon, ResetIcon, CopyIcon, CheckIcon, CheckboxIcon, GavelIcon, EyeIcon, TargetIcon, AlertTriangleIcon, CheckCircleIcon, FileTextIcon, StarIcon, ChevronDownIcon, BinaryIcon, ClipboardTextIcon } from './icons';
import { generatePdfReport } from '../services/geminiService';
import { encodeReport, toBase64 } from '../services/reportSerializer';
...
export const AnalysisDisplay: React.FC<AnalysisDisplayProps> = ({ result, fileName, onReset }) => {
  ...
  const [isBinCopied, setIsBinCopied] = useState(false);
  ...
  const handleDownloadBinReport = useCallback(() => {
    const binaryData = encodeReport(result);
    if (binaryData) {
        ...
        a.download = `${safeFileName}_report.verum.bin`;
  ...
  const handleCopyBinReport = useCallback(() => {
    const binaryData = encodeReport(result);
    if (binaryData) {
        const base64 = toBase64(binaryData);
        navigator.clipboard.writeText(base64).then(() => {
            setIsBinCopied(true);
            setTimeout(() => setIsBinCopied(false), 2500);
        }).catch(err => console.error('Failed to copy binary data: ', err));
    }
  }, [result]);
  ...
  return (
    <div className="animate-fade-in space-y-8">
      ...
      <div className="mt-8 text-center flex flex-col items-center justify-center gap-4">
        ...
        <div className="flex items-center space-x-2 pt-3 mt-3 border-t border-gray-800 w-full max-w-md mx-auto">
            <p className="text-xs text-gray-500 font-mono flex-1 text-left">V5 Protocol:</p>
            <button onClick={handleDownloadBinReport} title="Download as .verum.bin" className="p-2 bg-gray-700/50 border border-gray-600 text-gray-300 rounded-md hover:bg-gray-700 hover:text-white transition-colors">
                <BinaryIcon className="w-5 h-5" />
            </button>
            <button onClick={handleCopyBinReport} title="Copy binary report as Base64" className="p-2 bg-gray-700/50 border border-gray-600 text-gray-300 rounded-md hover:bg-gray-700 hover:text-white transition-colors">
                {isBinCopied ? <CheckIcon className="w-5 h-5 text-green-400" /> : <ClipboardTextIcon className="w-5 h-5" />}
            </button>
        </div>
      </div>
    </div>
